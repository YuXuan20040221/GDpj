...
def check_and_correct_pts(auto_pts, manual_pts, threshold=30):
    """
    如果自動偵測點和手動點距離超過 threshold，
    就用手動點替代，否則保留自動點。
    """
    corrected_pts = []
    for a, m in zip(auto_pts, manual_pts):
        dist = np.linalg.norm(a - m)
        if dist > threshold:
            corrected_pts.append(m)
        else:
            corrected_pts.append(a)
    return np.array(corrected_pts, dtype=np.float32)
...
def processing(img):
    ...
    auto_pts = detect_road_corners(img)
    if auto_pts is None:
        src_pts = manual_pts
    else:
        src_pts = check_and_correct_pts(auto_pts, manual_pts, threshold=50)
